class PuzzleFive {
  ...
  partOne() {
    ...
    parsed.forEach((seed) => {
      if (seed.location < minLocation) minLocation = seed.location
    })
    return minLocation
  }
  ...
  partTwo() {
    ...
    const waterRanges = this.splitRanges(
      fertilizerRanges,
      'fertilizer-to-water',
    )
    const lightRanges = this.splitRanges(waterRanges, 'water-to-light')
    const temperatureRanges = this.splitRanges(
      lightRanges,
      'light-to-temperature',
    )
    const humidityRanges = this.splitRanges(
      temperatureRanges,
      'temperature-to-humidity',
    )
    const locationRanges = this.splitRanges(
      humidityRanges,
  ...
  parseInput(input) {
    ...
    mapGroups.slice(1).forEach((group) => {
      ...
      data.slice(1).forEach((mapper) => {
        const [destination, source, length] = mapper
          .split(' ')
  ...
  getMappedValue(mapping, value) {
    ...
    mapping.forEach((mapping) => {
      ...
      if (
        value >= mappedNumber &&
        value <= mappedNumber + (mapping.length - 1)
      ) {
